...
<style>
  ...
  .fp-chr { width: 100%; max-width: 250px; height: auto; display: block; overflow: visible; font-family: "Roboto", sans-serif; justify-self: center; }
  ...
  .fp-card { display: grid; gap: 4px; padding: 12px 14px; border-radius: 8px; background: var(--fp-card); border: 1px solid var(--global-divider-color, rgba(0,0,0,0.1)); min-height: 64px; align-content: center; }
...
<div class="fp" id="fp">
  ...
  <div class="fp-side">
    <svg class="fp-chr" id="fp-chr" viewBox="0 0 250 470" role="img" aria-label="My interests drawn as one chromosome"></svg>
...
(function () {
  ...
  const CX = 78, HW = 24, TOP = 22, CEN = 150, BOT = 440, WAIST = 9, NECK = 26;
  ...
  rows.forEach(([y0, y1, id], i) => {
    ...
    if (b) {
      ...
      c += `<text class="bl" x="${CX - HW - 10}" y="${(y0 + y1) / 2 + 3.5}" text-anchor="end">${b.label}</text>`;
      c += `<line class="tick" x1="${CX + HW}" x2="${CX + HW + 14}" y1="${(y0 + y1) / 2}" y2="${(y0 + y1) / 2}"/>`;
      c += `<text class="bn" data-b="${id}" x="${CX + HW + 19}" y="${(y0 + y1) / 2 + 4}">${b.name}</text>`;
    }
  });
